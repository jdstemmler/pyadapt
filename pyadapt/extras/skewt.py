...
def wind_to_uv(wspd=None, wdir=None):
    """
    ...
    polar_dir = 90 - wdir
    polar_dir_rad = numpy.radians(polar_dir)

    u = -wspd * numpy.cos(polar_dir_rad)
    v = -wspd * numpy.sin(polar_dir_rad)
...
def plot_wind(fig, bx, u, v, a, skip=50,
              vertical_units="Altitude"):
    ...
    if vertical_units.lower() == "altitude":
        vert = -1*log10(h_to_p(a[::skip])/100.)
    elif vertical_units.lower() == "pressure":
        vert = -1*log10(a[::skip])

    bx.barbs(numpy.zeros(len(a[::skip])),
             vert,
             u[::skip],
             v[::skip])
...
def plot(pressure=None, altitude=None, profiles=None,
               u_wind=None, v_wind=None, wind_dir=None, wind_spd=None,
               colors=('r', 'b', 'k', 'g'),
               **kwargs):

    if pressure is None:
        if altitude is not None:
            pressure = h_to_p(altitude) / 100.
        elif altitude is None:
            raise TypeError("Both 'pressure' and 'altitude' cannot be NONE")

    if u_wind is not None and v_wind is not None:
        assert len(u_wind) == len(v_wind)

    elif wind_dir is not None and wind_spd is not None:
        assert len(wind_dir) == len(wind_spd)
        u_wind, v_wind = wind_to_uv(wspd=wind_spd, wdir=wind_dir)

    if profiles is None:
        raise TypeError("No Profiles to Plot")
    elif profiles is not None:
        if not isinstance(profiles, dict):
            raise TypeError("Variable 'profiles' must be a Dictionary")

    wind_interval = kwargs.pop("wind_interval", 50)
    skew = kwargs.pop("skew", 100)
    wind_vert = kwargs.pop("wind_vert", "pressure")
    if altitude is None:
        wind_vert = "pressure"

    fig, ax, bx = skewt_axes(skew=skew, **kwargs)

    for i, (k, v) in enumerate(profiles.iteritems()):
        fig, ax = plot_profile(fig, ax, v, pressure, c=colors[i], label=k, skew=skew)

    if wind_vert.lower() == "pressure":
        fig, bx = plot_wind(fig, bx, u_wind, v_wind, pressure,
                            skip=wind_interval, vertical_units=wind_vert)
    elif wind_vert.lower() == "altitude":
        fig, bx = plot_wind(fig, bx, u_wind, v_wind, altitude,
                            skip=wind_interval, vertical_units=wind_vert)
